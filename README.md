...
![refactoring_img](https://i.ibb.co/5GgSX05/2023-12-12-8-32-03.png)

| 강의명             | 수강 및 실습기간 |
| ------------------ | ---------------- |
| 리팩토링의 모든 것 | 9/30 - 11-8      |
...
- 타입과 같이 하나의 속성을 구별하기 위해 서브클래스를 만드는 것은 과하다.

### 12-8 : 슈퍼클래스 추출하기

- 공통 속성들이 있다면 슈퍼클래스로 만들 수 없을까 고민하자.

### 12-9 : 계층 합치기

- 처음부터 미래지향적 코드보다는, 나중에 필요하면 파생적인 코드로 만들자.

### 12-10 : 위임!

- 상속은 한 가지만 상속할 수 있기 때문에 최대한 남겨두고, 위임을 먼저 사용하자.
- 상속은 수직적인 구조에서 코드를 재사용, 확장한다. 필요없는 메서드까지 상속하게 되면 오히려 유지보수가 어렵고, 의미가 안 맞을 수 있다.
- 위임은 필요한 부품을 주입받아서 사용, 활용한다.
